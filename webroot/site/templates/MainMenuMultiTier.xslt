...
<xsl:variable name="tier1Id">
    <xsl:call-template name="getHomePageId"/>
</xsl:variable>
<xsl:variable name="tier2Id">
    <xsl:choose>
        <xsl:when test="/Pages/Page[@PageId=$currentID]/@TierLevel=1"></xsl:when>
        <xsl:when test="/Pages/Page[@PageId=$currentID][@TierLevel=2][@ParentId=$tier1Id]">
            <xsl:value-of select="$currentID"/>
        </xsl:when>
        <xsl:when test="/Pages/Page[@PageId=$currentID]/preceding-sibling::Page[@TierLevel=1][1]/@PageId=$tier1Id">
            <xsl:value-of select="/Pages/Page[@PageId=$currentID]/preceding-sibling::Page[@TierLevel=2][1]/@PageId"/>
        </xsl:when>
        <xsl:otherwise></xsl:otherwise>
    </xsl:choose>
</xsl:variable>
    <xsl:variable name="tier3Id">
        <xsl:choose>
            <xsl:when test="/Pages/Page[@PageId=$currentID]/@TierLevel=1"></xsl:when>
            <xsl:when test="/Pages/Page[@PageId=$currentID]/@TierLevel=2"></xsl:when>
            <xsl:when test="/Pages/Page[@PageId=$currentID]/@TierLevel=3">
                <xsl:value-of select="$currentID"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="/Pages/Page[@PageId=$currentID]/preceding-sibling::Page[@TierLevel=3][1]/@PageId"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
	...
	<li>
		<a>
            <xsl:attribute name="id">
                <xsl:value-of select="concat('ow_mainNav_',@PageId)"/>
            </xsl:attribute>
            <xsl:call-template name="formatAddress"/>
            <xsl:if test="@PageId=$tier2Id">
                <xsl:attribute name="class">current</xsl:attribute>
            </xsl:if>
            <xsl:if test="@PageId=$tier3Id">
                <xsl:attribute name="class">current</xsl:attribute>
            </xsl:if>
            <xsl:value-of select="@NavText"/>
		</a>
		<ul>
			<xsl:apply-templates select="following-sibling::Page[@ParentId=$pageId and not(contains(@Hidden, $navigation) or contains(@HiddenByParent,$navigation) or @Locked='true' or @LockedByParent='true')]"/>
		</ul>
	</li>
</xsl:template>
</xsl:stylesheet>