...
<xsl:variable name="tier1Id"><xsl:choose>
	<xsl:when test="/Pages/Page[@PageId=$currentID]/@TierLevel=0"><xsl:value-of select="$currentID"/></xsl:when>
	<xsl:otherwise><xsl:value-of select="/Pages/Page[@PageId=$currentID]/preceding-sibling::Page[@TierLevel=0][1]/@PageId"/></xsl:otherwise>
</xsl:choose></xsl:variable>

<xsl:variable name="tier2Id"><xsl:choose>
	<xsl:when test="/Pages/Page[@PageId=$currentID]/@TierLevel=0"></xsl:when>
...
<xsl:template match="/Pages">
	<xsl:if test="Page[@PageId=$tier2Id] and Page[@TierLevel=2 and @ParentId=$tier2Id and not(contains(@Hidden, $navigation) or contains(@HiddenByParent,$navigation) or @Locked='true' or @LockedByParent='true')]">
	<div id="ow_secoNav">
		<ul class="ow_secoNavList">
		<xsl:apply-templates select="Page[@TierLevel=2 and @ParentId=$tier2Id and not(contains(@Hidden, $navigation) or contains(@HiddenByParent,$navigation) or @Locked='true' or @LockedByParent='true')]"/>
		</ul>
	</div>
	</xsl:if>
</xsl:template>

<xsl:template match="Page[@TierLevel=2]">
	<li><a>
	...
	<ul>
		<xsl:apply-templates select="parent::*/Page[@TierLevel=3 and @ParentId=$tier3Id and not(contains(@Hidden, $navigation) or contains(@HiddenByParent,$navigation) or @Locked='true' or @LockedByParent='true')]"/>
	</ul>
	</xsl:if>
	</li>
</xsl:template>
